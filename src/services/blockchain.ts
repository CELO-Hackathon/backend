...
export class BlockchainService {
  ...
  //     // ✅ NEW APPROACH: Reconstruct the hash the same way the contract does
      
  //     // Get the TRANSFER_REQUEST_TYPEHASH (same as contract)
  ...
  //     // Hash the struct (same as contract's getTransferHash)
  ...
  //     // Construct the digest (same as contract's _verifySignature)
  ...
  //     // Check vault balance of recovered address
  ...
  async executeTransfer(
    ...
    try {
      ...
      // ✅ Calculate TRANSFER_REQUEST_TYPEHASH exactly like contract
      const TRANSFER_REQUEST_TYPEHASH = keccak256(
        encodePacked(
          ['string'],
          ['TransferRequest(address recipient,uint256 amount,uint256 nonce,uint256 deadline)']
      ...
      // ✅ Recover address using viem's recoverAddress (same as ecrecover in Solidity)
